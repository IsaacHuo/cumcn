...
\subsubsection{模型与求解}
问题一中全天输入已知，决策是144段购电量与充放电量。在共同物理约束的基础上，令紧急购电为零，并要求电池日末恢复初值：
\begin{equation}
\min C_1=\sum_{t=1}^{144}p_tg_t,\qquad E_0=E_{144}=6000.
\end{equation}
模型同时考虑各时段价格与后续能量需求，无需预先指定几点充电、几点放电。求解时先最小化费用，再在费用等价的方案中选取总充放电量较低者。所得策略没有同时充放电，且全部设备约束满足要求。
