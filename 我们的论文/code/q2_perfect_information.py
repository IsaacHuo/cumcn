...
def main():
    ...
    price=np.tile(price,334);t=np.arange(n);nvar=5*n+1
    ...
    np.savez_compressed(ROOT/'results/q2/perfect_information.npz',plan_kwh=x[:n].reshape(-1,N),charge_kwh=x[n:2*n].reshape(-1,N),discharge_kwh=x[2*n:3*n].reshape(-1,N),unused_kwh=x[3*n:4*n].reshape(-1,N),soc_kwh=x[4*n:])
    (ROOT/'results/q2/perfect_information.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(result,ensure_ascii=False))
